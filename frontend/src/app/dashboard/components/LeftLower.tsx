...
export default function LeftLower({ count, onSearch, activeApp, onBack, onUserInfoData, onPayroll, onAccessControl, apps = [], onAppClick, searchVal = "", onSearchChange }: Props) {
  ...
  const comboBox = (
    <div ref={dropdownRef} style={{ position: "relative", flex: 1 }}>
      ...
      {showDropdown && filtered.length > 0 && (
        <div style={{ position: "absolute", bottom: "100%", left: 0, right: 0, background: "white", border: "0.25px solid black", maxHeight: "200px", overflowY: "auto", zIndex: 50 }}>
          {filtered.map(app => (
            <div
              key={app.appname}
              onClick={() => { onAppClick?.(app.appname); onSearchChange?.(app.buttonname); setShowDropdown(false); }}
  ...
  if (isPortrait) {
    return (
      <div style={{ height: "100%", display: "flex", flexDirection: "row", alignItems: "center", gap: "8px", padding: "0 10px", overflow: "hidden" }}>
        {activeApp === "2fa" ? mainBtn :
          activeApp ? (
            <>
